...
from .serializer import PersonasGetName,PersonasCreateSerializer
...
class PersonasApi(APIView):
    ...
    def get(self, request):
        ...
        serializer = PersonasGetName(gente, many=True)
        return Response(serializer.data,status=status.HTTP_200_OK)

    def post(self, request):
        bato=PersonasCreateSerializer(data=request.data)
        if bato.is_valid():
            bato.save()
